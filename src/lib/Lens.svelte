<script lang="ts">
    ...
    let { children, lensEffect = $bindable() } = $props();

    let zoomFactor = $state(1.5);
    let lensSize = $state(170);
    let isStatic = $state(false);
    let position = $state({ x: 200, y: 150 });
...
<div
    class="relative overflow-hidden rounded-2xl z-20 h-full w-full {lensEffect ? 'cursor-none' : ''}"
    onmouseenter={() => (hovering = lensEffect ? true : false)}
